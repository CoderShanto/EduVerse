...
class LeaderboardController extends Controller
{
    ...
    {
        // ✅ Load all users with student role (or include instructors if you want)
        ...
        // updates count per user (innovation updates)
        $updatesByUser = IdeaUpdate::selectRaw('user_id, COUNT(*) as cnt')
            ->groupBy('user_id')
            ->pluck('cnt', 'user_id');

        // showcased ideas (completed) count per idea + score
        $showcases = IdeaShowcase::select('idea_id', 'score')->get()->keyBy('idea_id');

        // member mapping: which users are in which idea
        $members = IdeaMember::select('idea_id', 'user_id')
            ->whereIn('idea_id', array_keys($showcases->toArray()) + $selectedIdeaIds)
            ->get();
