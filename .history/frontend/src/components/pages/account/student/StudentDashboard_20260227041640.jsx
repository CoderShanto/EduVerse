    ...
    <div className='sd-card'>
      ...
      <div className='sd-prog-track' style={{ marginBottom: '1rem' }}>
        <div
          ...
          style={{ width: `${(done / DAILY_GOALS.length) * 100}%` }}
        />
      </div>
...
const StudentDashboard = () => {
  ...
  // ✅ FIX: Use token the same way you do in WatchCourse (Config token)
  // Also keep fallback from context if you have it.
  const authToken =
    user?.token ||
    user?.user?.token ||
    configToken
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // ✅ Debug log (helps you confirm API returns enrollments/progress)
    ...
    // ✅ Always attempt fetch if we have any token (Config or context)
    if (authToken) load()
