...
export default function AdminCategories() {
  ...
  const toggleStatus = async (id) => {
    try {
      ...
      const result = await res.json();
      if (result?.status === 200) { toast.success("Status toggled!"); fetchCategories(meta.current_page); }
      ...
      <div className="acc-root">
        ...
        <div className="acc-card" style={{ marginBottom: 0 }}>
          <div className="acc-card-header">
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: "#94a3b8" }}>{items.length} result{items.length !== 1 ? "s" : ""}</span>
          </div>
