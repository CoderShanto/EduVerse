...
const css = `
  ...
  /* course card wrapper — grid view */
  .ml-grid .ml-card-wrap { }

  /* course card wrapper — list view */
  .ml-list .ml-card-wrap {
    grid-column: 1 / -1;
  }
  ...
  /* card shell — wraps CourseEnrolled */
  .ml-card-shell {
    background: #fff;
    border-radius: 16px;
    border: 1.5px solid #ede9e1;
  ...
  /* inline progress bar inside card */
...
const getProgress = (enrollment) =>
  enrollment?.progress ?? enrollment?.completion_percentage ?? 0
            ...
            <div className='col-lg-9 mt-4 mt-lg-0'>
              ...
              {!loading && total > 0 && (
                <div className='ml-summary'>
                  ...
                  <div className='ml-summary-item'>
                    <div className='ml-summary-dot' style={{ background: '#ede9e1' }} />
              ...
              {!loading && total > 0 && (
                <div className='ml-toolbar'>
                  ...
                  <div className='ml-view-toggle'>
                    <button className={`ml-view-btn ${view === 'grid' ? 'active' : ''}`} onClick={() => setView('grid')} title='Grid view'>
                      ⊞
                    </button>
                    <button className={`ml-view-btn ${view === 'list' ? 'active' : ''}`} onClick={() => setView('list')} title='List view'>
                      ☰
                    </button>
                  </div>
                </div>
                  ...
                  {filtered.map((enrollment) => {
                    ...
                    return (
                      <div key={enrollment.id} className='ml-card-shell'>
                        ...
                        {/* Status ribbon */}
                        <div className={`ml-ribbon ${status}`}>
                          {STATUS_LABELS[status]}
                        </div>

                        {/* Existing CourseEnrolled component */}
                        ...
                        {/* CTA */}
                        <a
                          href={`/account/course/${enrollment?.course?.slug ?? enrollment?.course_id}`}
