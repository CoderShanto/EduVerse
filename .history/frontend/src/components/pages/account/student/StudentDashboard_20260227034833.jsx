...
═══════════════════════════════════════ */
const QUICK_LINKS = [
  { to: '/account/my-learning',  icon: '📚', label: 'My Learning' },
  { to: '/account/profile',      icon: '👤', label: 'My Profile' },
  { to: '/account/change-password', icon: '🔒', label: 'Change Password' },
...
const statusChip = (pct) => {
  ...
  if (p >= 100) return { label: 'Completed', className: 'sd-chip', emoji: '🏆' }
  if (p > 0) return { label: 'In progress', className: 'sd-chip', emoji: '▶️' }
  return { label: 'Not started', className: 'sd-chip', emoji: '🚀' }
}
...
const Donut = ({ value = 0 }) => {
  ...
  return (
    <svg className='sd-donut' viewBox='0 0 60 60' aria-label={`Progress ${pct}%`}>
      <circle cx='30' cy='30' r={r} fill='none' stroke='#ede9e1' strokeWidth='8' />
      <circle
        cx='30'
        cy='30'
        r={r}
      ...
      <defs>
        <linearGradient id='grad' x1='0' y1='0' x2='1' y2='0'>
          <stop offset='0%' stopColor='#c17d3c' />
          <stop offset='100%' stopColor='#e8a75a' />
        </linearGradient>
      </defs>
...
const DailyGoals = () => {
  ...
  return (
    <div className='sd-card'>
      ...
      <div className='sd-mini-track' style={{ marginBottom: '1rem', height: 10 }}>
        <div
          className='sd-mini-fill'
          style={{
...
const StudentDashboard = () => {
  ...
  const avgProgress = useMemo(() => {
    ...
    const sum = progressCourses.reduce((a, c) => a + (Number(c?.progress) || 0), 0)
    return Math.round(sum / progressCourses.length)
  }, [progressCourses])
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // supports both:
        // {status:200, stats:{...}}
        // and {status:200, role:'student', stats:{...}}
    ...
    <Layout>
      ...
      <div className='sd-root'>
        <div className='container'>
          ...
          <nav className='sd-bc'>
            <Link to='/account/student/dashboard'>Account</Link>
            ...
            <div className='col-lg-9'>
              ...
              <div className='sd-greeting mb-4'>
                <div className='sd-greeting-left'>
                  ...
                  <div className='sd-greeting-text'>
                    <h2>Good {getTimeGreeting()}, {displayName}! 👋</h2>
              ...
              {loading ? (
                ...
                <>
                    ...
                    <div className='col-md-4'>
                      <div className='sd-stat-card'>
                        <div>
                          ...
                          <div className='sd-num'>{avgProgress}%</div>
                          <div className='sd-label'>Average Progress</div>
                        </div>
                        <span className='sd-stat-link' style={{ cursor:'default', color:'#9a8f7e' }}>Stay consistent!</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* REAL Progress + Quick links */}
                  <div className='row g-3 mb-4'>
                    <div className='col-md-7'>
                      <div className='sd-card h-100'>
                        <p className='sd-section-title'>Your Course Progress</p>

                        {progressCourses.length === 0 ? (
                          <div style={{ padding: '0.5rem 0', color: '#7a6f60' }}>
                            No course progress found yet. Enroll in a course and start learning!
                            <div style={{ marginTop: '.8rem' }}>
                              <Link className='sd-go-btn' to='/courses'>Browse Courses</Link>
                            </div>
                          </div>
                        ) : (
                          progressCourses.map((c) => {
                            const pct = Math.max(0, Math.min(100, Number(c?.progress) || 0))
                            const chip = statusChip(pct)
                            const courseId = c?.course_id

                            return (
                              <div key={courseId} className='sd-progress-item'>
                                <div className='sd-course-badge'>{getBadgeText(c?.title)}</div>

                                <div className='sd-course-meta'>
                                  <div className='sd-course-title'>{c?.title}</div>
                                  <div className='sd-course-sub'>
                                    <span>{Number(c?.completed_lessons) || 0}/{Number(c?.total_lessons) || 0} lessons</span>
                                    <span>•</span>
                                    <span className={chip.className}>{chip.emoji} {chip.label}</span>
                                  </div>

                                  <div className='sd-mini-track'>
                                    <div className='sd-mini-fill' style={{ width: `${pct}%` }} />
                                  </div>
                                </div>

                                <div className='sd-progress-actions'>
                                  <Donut value={pct} />
                                  <Link className='sd-go-btn' to={`/account/watch-course/${courseId}`}>
                  ...
                  {/* Streak (optional) */}
                  <div className='sd-card mt-4'>
                    <p className='sd-section-title'>Streak</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem' }}>
                      <div style={{ fontSize: '1.6rem' }}>🔥</div>
                      <div>
                        <div style={{ fontWeight: 900, color: '#1a1610' }}>{streak} days</div>
                        <div style={{ fontSize: '.8rem', color: '#7a6f60' }}>
                          (If you haven&apos;t implemented real streak logic yet, it will show 0)
                        </div>
                      </div>
                    </div>
                  </div>
