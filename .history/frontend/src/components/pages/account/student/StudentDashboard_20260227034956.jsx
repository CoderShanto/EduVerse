...
const css = `
  ...
  .sd-donut-legend-item:last-child { margin-bottom: 0; }
  .sd-donut-dot {
    width: 10px; height: 10px;
    border-radius: 50%;
    flex-shrink: 0;
  }
  .sd-donut-legend-name {
    flex: 1;
    white-space: nowrap;
    overflow: hidden;
  ...
  .sd-bar-col:hover .sd-bar-fill { filter: brightness(1.1); }
  .sd-bar-tooltip {
    display: none;
    position: absolute;
    top: -32px;
  ...
  .sd-bar-col:hover .sd-bar-tooltip { display: block; }
  .sd-bar-fill {
    width: 100%;
  ...
  .sd-empty-progress span { font-size: 2rem; display: block; margin-bottom: 0.5rem; }
  ...
  @media (max-width: 768px) {
    ...
    .sd-donut-wrap { flex-direction: column; align-items: center; }
  }
`
...
// Color palette for chart bars / donut slices
const CHART_COLORS = [
  '#c17d3c', '#3a5298', '#1a8c52', '#9b2335', '#5c3d8f',
  '#1a7a8a', '#d4821a', '#2d6e3e', '#a0522d', '#2a4a7f',
]
...
/* ═══════════════════════════════════════
   SVG DONUT CHART
   Renders a donut chart showing each
   course's progress as an arc segment.
═══════════════════════════════════════ */
const DonutChart = ({ courses }) => {
  const SIZE = 130
  const cx = SIZE / 2
  const cy = SIZE / 2
  const R  = 48          // outer radius
  const r  = 32          // inner radius (hole)
  const strokeW = R - r  // = 16
  ...
  // Each course gets a proportional arc based on its progress value
  // We render them as stacked arcs of the same ring, each with its own color.
  // The total angle = 360°. We split it equally per course, fill each arc by progress%.

  const sliceAngle = courses.length > 0 ? 360 / courses.length : 0
  ...
  // Build SVG arc path for a slice from startAngle to endAngle
  const arcPath = (startDeg, endDeg, outerR, innerR) => {
    const clamp = Math.min(endDeg - startDeg, 359.99)
    const end = startDeg + clamp
    const o1 = polar(startDeg, outerR)
    const o2 = polar(end, outerR)
    const i1 = polar(end, innerR)
  ...
  return (
    <div className='sd-donut-wrap'>
      ...
      >
        {/* background ring */}
        {courses.map((_, idx) => {
          const start = idx * sliceAngle + 1
          const end   = (idx + 1) * sliceAngle - 1
          return (
            <path
              key={`bg-${idx}`}
              d={arcPath(start, end, R, r)}
              fill='#ede9e1'
            />
          )
        })}
        ...
        {courses.map((c, idx) => {
          const start   = idx * sliceAngle + 1
          const fillEnd = start + ((sliceAngle - 2) * c.progress / 100)
          if (c.progress === 0) return null
          return (
            <path
              key={`fill-${idx}`}
              d={arcPath(start, fillEnd, R, r)}
        ...
        <text
          x={cx} y={cy - 6}
          textAnchor='middle'
          className='sd-donut-center-label'
          fontSize='18'
          fontWeight='700'
          fill='#1a1610'
      ...
      <div className='sd-donut-legend'>
        {courses.map((c, idx) => (
          <div key={idx} className='sd-donut-legend-item'>
            <div
              className='sd-donut-dot'
              style={{ background: CHART_COLORS[idx % CHART_COLORS.length] }}
            />
            <span className='sd-donut-legend-name' title={c.title}>{c.title}</span>
            <span className='sd-donut-legend-pct'>{c.progress}%</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className='sd-bar-chart'>
      {courses.map((c, idx) => {
        ...
        const barH   = mounted ? `${Math.max(pct, 4)}%` : '4%'

        // Truncate label: first word or first 6 chars
        const shortName = c.title.split(' ').slice(0, 2).join(' ')

        return (
          <div key={idx} className='sd-bar-col'>
            <div className='sd-bar-tooltip'>
              {c.title}<br/>{c.completed_lessons}/{c.total_lessons} lessons · {c.progress}%
            </div>
            <div
            ...
            />
            <div className='sd-bar-label' title={c.title}>{shortName}</div>
          </div>
        )
      })}
    </div>
...
/* ═══════════════════════════════════════
   COURSE PROGRESS CHART PANEL
   Shows real data from the API with
   toggle between Donut and Bar views.
═══════════════════════════════════════ */
const CourseProgressChart = ({ courses }) => {
  const [view, setView] = useState('bar') // 'bar' | 'donut' | 'list'

  if (!courses || courses.length === 0) {
    return (
      <div className='sd-empty-progress'>
        <span>📭</span>
        No enrolled courses yet.<br />
        <Link to='/courses' style={{ color: '#c17d3c', fontWeight: 700, fontSize: '0.82rem' }}>
          Browse courses →
        </Link>
      </div>
    )
  ...
  return (
    <div className='sd-chart-wrap'>
      ...
      <div className='sd-chart-tabs'>
        <button
          className={`sd-chart-tab ${view === 'bar' ? 'active' : ''}`}
          onClick={() => setView('bar')}
        >
          Bar
        </button>
        <button
          className={`sd-chart-tab ${view === 'donut' ? 'active' : ''}`}
          onClick={() => setView('donut')}
        >
          Donut
        </button>
        <button
          className={`sd-chart-tab ${view === 'list' ? 'active' : ''}`}
      ...
      {/* Chart content */}
      {view === 'bar' && <BarChart courses={courses} />}

      {view === 'donut' && <DonutChart courses={courses} />}

      {view === 'list' && courses.map((c, i) => (
        <div key={i} className='sd-course-row'>
          <div
            className='sd-course-thumb'
            style={{ background: `${CHART_COLORS[i % CHART_COLORS.length]}22` }}
          >
            <span style={{ fontSize: '1rem' }}>
              {c.course_small_image
                ? <img src={c.course_small_image} alt='' style={{ width: 40, height: 40, borderRadius: 10, objectFit: 'cover' }} />
                : '📘'
              }
            </span>
          </div>
          <div className='sd-course-info'>
            <div className='sd-course-name'>{c.title}</div>
      ...
      <Link to='/account/my-learning' className='sd-stat-link' style={{ marginTop: '0.8rem', display: 'inline-flex' }}>
        All courses →
      </Link>
    </div>
  )
...
const StudentDashboard = () => {
  ...
  // progress_courses from API: array of { course_id, title, slug, course_small_image, total_lessons, completed_lessons, progress }
  const progressCourses = stats?.progress_courses ?? []
            ...
            <div className='col-lg-9'>
              ...
              <div className='sd-greeting mb-4'>
                <div className='sd-greeting-left'>
                  ...
                  <div className='sd-greeting-text'>
                    <h2>
                      Good {getTimeGreeting()}, {user.user.name}! 👋
                    </h2>
              ...
              {loading ? (
                ...
                <>
                  ...
                  {/* ── Course Progress Chart (REAL DATA) + Quick Links ── */}
                  <div className='row g-3 mb-4'>
                    <div className='col-md-7'>
                      <div className='sd-card h-100'>
                        <p className='sd-section-title'>Course Progress</p>
                        <CourseProgressChart courses={progressCourses} />
                      </div>
                    </div>
                    <div className='col-md-5'>
                      <div className='sd-card h-100'>
