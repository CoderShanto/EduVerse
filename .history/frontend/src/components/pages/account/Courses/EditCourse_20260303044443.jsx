...
const EditCourse = () => {
  ...
  const [featureLoading, setFeatureLoading] = useState(false); // ✅ NEW
  ...
  // ✅ NEW: Toggle is_featured 'yes' <-> 'no'
  const toggleFeatured = async () => {
    const isFeatured = String(course.is_featured).toLowerCase() === "yes" || String(course.is_featured) === "1";
    const newVal = isFeatured ? "no" : "yes";
    ...
    try {
      ...
      const result = await res.json();
      if (result.status == 200) {
        toast.success(newVal === "yes" ? "⭐ Course marked as featured" : "Course removed from featured");
        ...
        <div className="container pb-5 pt-3">
          ...
          <div className="row">
            <div className="col-md-12 mt-5 mb-3">
              ...
              {/* Helpful hint shown when course is unpublished */}
              {!isPublished && courseLoaded && (
                <div className="alert alert-warning mt-3 mb-0 py-2 px-3" style={{ fontSize: 13 }}>
                  ⚠️ This course is <strong>unpublished</strong>. To publish it, make sure you have added
                  at least <strong>one chapter</strong> and <strong>one lesson with a video</strong>.
                </div>
              )}
            </div>
