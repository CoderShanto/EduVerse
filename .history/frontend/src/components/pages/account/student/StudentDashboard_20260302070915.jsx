...
const LiveClock = () => {
  ...
  return (
    <div className='sd-clock'>
      <div className='sd-clock-time'>{hh}:{mm}<span className='sd-secs'>:{ss}</span></div>
      <div className='sd-clock-date'>{now.toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div>
...
const StudentDashboard = () => {
  ...
  const initials = useMemo(() => (userName||'Student').split(' ').map(w=>w[0]).join('').slice(0,2).toUpperCase(),[userName])
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        setLoading(true)
        const sR = await fetch(`${apiUrl}/dashboard/stats`,{headers:{Accept:'application/json',Authorization:`Bearer ${authToken}`}})
        const sJ = await sR.json()
        setStats(sJ?.status===200 ? sJ.stats : null)

        const pR = await fetch(`${apiUrl}/problems?page=1`,{headers:{Accept:'application/json',Authorization:`Bearer ${authToken}`}})
        const pJ = await pR.json()
        setLatestProblems((pJ?.status===200?(pJ.data?.data||[]):[]).slice(0,4))

        const shR = await fetch(`${apiUrl}/showcases?page=1`,{headers:{Accept:'application/json',Authorization:`Bearer ${authToken}`}})
    ...
    if (authToken) loadAll(); else setLoading(false)
  }, [authToken])
    ...
    <Layout>
      ...
      <div className='sd-root'>
        <div className='container sd-inner'>
          <nav className='sd-bc'>
            <Link to='/account/dashboard'>Account</Link><span>›</span><span>Student Dashboard</span>
          </nav>
          <div className='row'>
            ...
            <div className='col-lg-9'>
              ...
              <div className='sd-greeting mb-4'>
                <div className='sd-greeting-deco d1'/><div className='sd-greeting-deco d2'/>
                ...
                <>
                  ...
                  <div className='row g-3 mb-4'>
                    <div className='col-12'>
                      <div className='sd-card'>
                        <p className='sd-section-title'><span className='sd-dot' style={{background:'var(--blue)'}}/>Course Progress</p>
                              ...
                              return (
                                <div key={`${c.course_id}-${i}`} className='sd-course-row'>
                                  <div className='sd-course-thumb'>
                                    {c.course_small_image ? <img src={c.course_small_image} alt={c.title}/> : <span>📘</span>}
                                  </div>
                                  <div className='sd-course-info'>
                                    <div className='sd-course-name'>{c.title||'Untitled course'}</div>
                                    <div className='sd-prog-track'><div className='sd-prog-fill' style={{width:`${pct}%`}}/></div>
                                    <div className='sd-course-pct'>{pct}% complete</div>
                                  </div>
                                  {c.course_id && <Link to={`${WATCH_ROUTE_BASE}/${c.course_id}`} className='sd-continue-btn'>Continue →</Link>}
                                </div>
                              )
                            })}
                            <Link to='/account/student/my-learning' className='sd-hub-link' style={{marginTop:'0.8rem'}}>All courses →</Link>
                          </>
                        ) : (
                          <div className='sd-muted'>No course progress yet. Start from <Link to='/account/student/my-learning'>My Courses</Link>.</div>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className='row g-3 mb-4'>
                    <div className='col-md-6'>
                      <div className='sd-card'>
                        <p className='sd-section-title'><span className='sd-dot' style={{background:'var(--yellow)'}}/>Innovation Spotlight</p>
                        {latestProblems.length === 0 ? (
                          <div className='sd-muted'>No problems yet. Post one in <Link to='/account/innovation'>Problem Hub</Link>.</div>
                        ) : (
                    ...
                    <div className='col-md-6'>
                      <div className='sd-card'>
                        <p className='sd-section-title'><span className='sd-dot' style={{background:'var(--green)'}}/>Showcase Spotlight</p>
                        {latestShowcases.length === 0 ? (
                          <div className='sd-muted'>No showcases yet. Build in <Link to='/account/innovation/my-teams'>My Teams</Link> and publish.</div>
                        ) : (
                          <div className='d-flex flex-column gap-2'>
