...
const css = `
  ...
  .sd-root { background: var(--bg); min-height: 100vh; font-family: var(--font); padding-bottom: 4rem; color: var(--text); position: relative; }
  .sd-blob-wrap { position: fixed; inset: 0; pointer-events: none; overflow: hidden; z-index: 0; }
  .sd-blob { position: absolute; border-radius: 50%; filter: blur(80px); opacity: 0.35; }
  ...
  .sd-blob-2 { width: 380px; height: 380px; background: radial-gradient(circle, #ffd5c5, #ffb3a0); bottom: 0; left: -80px; animation: blob-float 14s ease-in-out infinite alternate-reverse; }
  .sd-blob-3 { width: 250px; height: 250px; background: radial-gradient(circle, #b5f0d8, #86efca); top: 40%; left: 40%; animation: blob-float 8s ease-in-out infinite alternate; }
  ...
  .sd-avatar { width: 64px; height: 64px; border-radius: 20px; background: rgba(255,255,255,0.2); backdrop-filter: blur(10px); border: 2px solid rgba(255,255,255,0.35); display: flex; align-items: center; justify-content: center; font-family: var(--font-serif); font-size: 1.5rem; font-style: italic; color: #fff; flex-shrink: 0; box-shadow: 0 8px 24px rgba(0,0,0,0.15); animation: pop-in 0.5s 0.15s cubic-bezier(0.34,1.56,0.64,1) both; }
  ...
  .sd-stat-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px; border-radius: 99px 99px 0 0; }
  ...
  .sd-stat-card::after { content: attr(data-num); position: absolute; right: -10px; bottom: -20px; font-family: var(--font-serif); font-size: 7rem; font-weight: 700; line-height: 1; opacity: 0.04; color: #000; pointer-events: none; user-select: none; }
  .sd-stat-icon { width: 48px; height: 48px; border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 1.4rem; margin-bottom: 1.2rem; }
  ...
  .sd-num { font-family: var(--font-serif); font-size: 3.2rem; font-weight: 700; line-height: 1; color: var(--text); margin-bottom: 0.3rem; }
  .sd-label { font-size: 0.8rem; color: var(--text2); font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; }
  .sd-stat-link { margin-top: auto; padding-top: 1rem; font-size: 0.78rem; font-weight: 700; text-decoration: none; display: inline-flex; align-items: center; gap: 5px; transition: gap 0.2s; }
  ...
  .sd-list-row-sub { font-size: 0.72rem; color: var(--text2); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 0.2rem; }
  .sd-pill { font-size: 0.65rem; padding: 3px 9px; border-radius: 999px; border: 1.5px solid var(--border); background: var(--white); color: var(--text2); font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }
  .sd-view-btn { font-size: 0.72rem; font-weight: 700; color: var(--purple); text-decoration: none; white-space: nowrap; display: inline-flex; align-items: center; gap: 4px; padding: 5px 10px; border: 1.5px solid #e0d5f5; border-radius: 9px; background: #f5f0ff; flex-shrink: 0; transition: all 0.2s; }
  .sd-view-btn:hover { background: var(--purple); color: #fff; border-color: var(--purple); }
  .sd-hub-link { font-size: 0.76rem; font-weight: 700; color: var(--blue); text-decoration: none; display: inline-flex; align-items: center; gap: 5px; margin-top: 0.8rem; opacity: 0.8; transition: gap 0.2s, opacity 0.2s; }
  ...
  .sd-skeleton { border-radius: var(--radius); height: 130px; background: linear-gradient(90deg,#e8ecff 0%,#f0f4ff 50%,#e8ecff 100%); background-size: 700px 100%; animation: shimmer 1.6s infinite; border: 1.5px solid var(--border); }
  @keyframes shimmer { 0% { background-position: -700px 0; } 100% { background-position: 700px 0; } }
  @keyframes sd-up { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
...
const buildImgCandidates = (raw, title) => {
  ...
  // already starts with /uploads or /storage
  if (s.startsWith("/uploads") || s.startsWith("/storage")) {
    return { candidates: [`${backendBase}${s}`, placeholder], placeholder };
  }

  // already like "uploads/..."
  if (s.startsWith("uploads/") || s.startsWith("storage/")) {
    return { candidates: [`${backendBase}/${s}`, placeholder], placeholder };
  }

  // filename only → try common folders (small first, then normal)
  const candidates = [
...
// ✅ Smart image that falls back automatically
                        ...
                        {progressCourses.length > 0 ? (
                          <>
                            {progressCourses.map((c, i) => {
                              ...
                              // ✅ IMPORTANT: try MANY keys (some APIs return different names)
                              const rawImg =
                                c.image_small_url ||
                                c.image_url ||
                              ...
                              return (
                                <div key={`${c.course_id || c.id}-${i}`} className="sd-course-row">
                                  ...
                                  {(c.course_id || c.id) && (
                                    <Link to={`${WATCH_ROUTE_BASE}/${c.course_id || c.id}`} className="sd-continue-btn">
                                      Continue →
                                    </Link>
                                  )}
                                </div>
