...
const Course = ({ course, customClasses }) => {
  // If Cloudinary, course.image should be a full https url.
  // Old DB rows may still have filenames or empty values.
  const imgSrc =
    course?.image && String(course.image).startsWith("http")
      ? course.image
      : course?.course_small_image && String(course.course_small_image).startsWith("http")
      ? course.course_small_image
      : `https://placehold.co/600x350?text=${encodeURIComponent(course?.title || "Course")}`;

  return (
    <div className={customClasses}>
      <div className="card border-0">
        <div className="card-img-top">
          <img src={imgSrc} alt={course?.title || "Course"} className="img-fluid" />
        </div>
          ...
          <div className="meta d-flex py-2">
            ...
            <div className="student ps-4">
              <div className="d-flex align-items-center">
                ...
                <div className="text ps-2">{course?.enrollments_count ?? 0}</div>
              </div>
            </div>
