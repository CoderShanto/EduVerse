...
{
    $items = \App\Models\IdeaShowcase::with([
            'idea.problem',
            'idea.user',
            'idea.updates' // IMPORTANT
        ])
        ->latest()
        ->paginate(12);

    // add resolved cover image
    $items->getCollection()->transform(function ($s) {

        $cover = $s->cover_image;

        // if cover_image missing -> try last update image
        if (!$cover && $s->idea && $s->idea->updates) {
            $imgUpdate = $s->idea->updates
                ->where('proof_type', 'image')
                ->whereNotNull('proof_url')
                ->last();

            if ($imgUpdate) {
                $cover = $imgUpdate->proof_url;
            }
        }

        $s->cover_image_resolved = $cover;
        return $s;
    });
