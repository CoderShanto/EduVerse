...
const css = `
  ...
  .aac-pill {
    ...
    border: 1px solid transparent; font-weight: 800; font-size: 12px;
  ...
  .aac-pill.featuredOn { background: #fff7ed; border-color: #fed7aa; color: #9a3412; }
  .aac-pill.featuredOn:hover { background: #ffedd5; box-shadow: 0 6px 18px rgba(154,52,18,.10); }
  .aac-pill.featuredOff { background: #f8fafc; border-color: #e2e8f0; color: #0f172a; }
  .aac-pill.featuredOff:hover { background: #f1f5f9; box-shadow: 0 6px 18px rgba(15,23,42,.08); }
  ...
  .aac-title { font-weight: 900; font-size: 14px; color: #111827; }
  .aac-sub { font-size: 12px; color: #6b7280; }
  .aac-badge { border-radius: 999px; padding: 6px 10px; font-size: 12px; font-weight: 900; }
`;
...
const AdminAllCourses = () => {
  ...
  const [featureChangingId, setFeatureChangingId] = useState(null);

  // ✅ handles yes/no, 1/0, true/false
  const isFeaturedValue = (val) => {
  ...
  const filtered = useMemo(() => {
    ...
    if (k) list = list.filter((c) => String(c.title || "").toLowerCase().includes(k));
  ...
  const toggleFeatured = async (course) => {
    try {
      ...
      if (result?.status === 200) {
        ...
        console.log("FEATURE TOGGLE ERROR =>", result);
        toast.error(result?.message || "Failed to update featured");
      }
    } catch (e) {
      console.log(e);
      toast.error("Network error");
    } finally {
      setFeatureChangingId(null);
    }
  };

  const statusBadge = (c) => {
    const ok = isPublishedValue(c.status);
    return (
      <span className={`aac-badge badge ${ok ? "bg-success" : "bg-secondary"}`}>
  ...
  const featuredBadge = (c) => {
    const ok = isFeaturedValue(c.is_featured);
    return (
      <span className={`aac-badge badge ${ok ? "bg-warning text-dark" : "bg-light text-dark"}`}>
        {ok ? "Yes" : "No"}
      </span>
    );
  };
        ...
        <div className="row">
          ...
          <div className="col-lg-9">
                    ...
                    <table className="table aac-table align-middle mb-0">
                      <thead>
                        <tr>
                          ...
                          <th style={{ width: 310 }} className="text-end">
                            Actions
                          </th>
                        </tr>
                      </thead>
                            ...
                            <tr key={c.id} className="aac-row">
                              ...
                              <td className="text-end">
                                <div className="aac-actions">
                                  <button
                                    type="button"
                                    className={`aac-pill ${featuredNow ? "featuredOn" : "featuredOff"}`}
                                    onClick={() => toggleFeatured(c)}
                                    disabled={featureChangingId === c.id}
                                  >
                                    {featureChangingId === c.id
                                      ? "⏳ Updating"
                                      : featuredNow
                                      ? "⭐ Featured"
                                      : "☆ Feature"}
            ...
            <div className="text-muted mt-2" style={{ fontSize: 12 }}>
              Featured supports DB values <code>yes/no</code> and <code>1/0</code>.
            </div>
          </div>
        </div>
      </div>
